"""Collector: LSF job 對回 case 的三種方式。

Arcx 送出的子 job 沒有可辨識的 job name, 所以這段對應邏輯是監控能不能
看到 LSF 真實狀態的關鍵。
"""
...
class AttachLsfTest(unittest.TestCase):
    ...
    def test_match_by_output_file(self):
        """方式 1: output_file 直接等於該 case 的 log —— 最可靠, 零額外 I/O。"""
        ...
        self.assertIsNotNone(obs.cases["case1"].lsf)
        self.assertEqual(obs.cases["case1"].lsf.job_id, "777")
        self.assertIsNone(obs.cases["case2"].lsf)

    def test_match_by_exec_cwd(self):
        """方式 2: job 的 cwd 落在該 case 的 run dir 底下。"""
        job = LsfJobView(job_id="888", state=LsfState.SSUSP,
                         exec_cwd=os.path.join(self.folder, "case2"))
        obs = self.collector.collect_index_run(self.folder, "1000", lsf_jobs=[job])
        self.assertEqual(obs.cases["case2"].lsf.job_id, "888")
        self.assertEqual(obs.cases["case2"].lsf.state, LsfState.SSUSP)

    def test_job_outside_run_folder_ignored(self):
        """別的 wave 的 job 不能被誤掛上來。"""
        job = LsfJobView(job_id="999", state=LsfState.RUN,
                         exec_cwd="/somewhere/else/case1")
        obs = self.collector.collect_index_run(self.folder, "1000", lsf_jobs=[job])
        self.assertIsNone(obs.cases["case1"].lsf)

    def test_parent_arcx_job_not_attached_to_cases(self):
        """在 index run folder 執行的 parent Arcx job 不能被掛到任何 case 上。

        它的 cwd 是所有 case 目錄的上層, 若用雙向前綴比對就會被掛到每一個
        case, 讓整張狀態表顯示同一個 job —— 比對不到還糟。
        """
    ...
    def test_match_by_log_head_path(self):
        """方式 3: log 開頭寫著執行路徑, job 的 cwd 落在那底下。"""
        job = LsfJobView(job_id="666", state=LsfState.RUN,
                         exec_cwd=os.path.join(self.folder, "case1", "sub"))
        obs = self.collector.collect_index_run(self.folder, "1000",
                                               lsf_jobs=[job])
        self.assertEqual(obs.cases["case1"].lsf.job_id, "666")
        self.assertIsNone(obs.cases["case2"].lsf)
...
class CollectWaveTest(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory()
        self.settings = Settings()
        self.collector = Collector(settings=self.settings)
    ...
    def test_collects_all_index_run_folders(self):
        ...
        make_index_run_folder(wave, "1001", [CaseSpec("case1"), CaseSpec("case2")])
